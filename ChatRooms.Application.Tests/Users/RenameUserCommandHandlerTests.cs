...
public sealed class RenameUserCommandHandlerTests
{
    private readonly Mock<IUserRepository> _userRepositoryMock;
    private readonly Mock<IUnitOfWork> _unitOfWorkMock;
    private readonly Mock<IDateTimeProvider> _dateTimeProviderMock;
    private readonly RenameUserCommandHandler _handler;
    private readonly User _existingUser;
    private readonly Guid _userId;

    public RenameUserCommandHandlerTests()
    {
    ...
    [Fact]
    public async Task Handle_ValidRename_ShouldReturnNewName()
    {
        var command = new RenameUserCommand(_userId, "NewName");

        var result = await _handler.Handle(command, CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal("NewName", result.Value);
    ...
    [Fact]
    public async Task Handle_SameName_ShouldNotCallSave()
    {
        var command = new RenameUserCommand(_userId, "Original");
